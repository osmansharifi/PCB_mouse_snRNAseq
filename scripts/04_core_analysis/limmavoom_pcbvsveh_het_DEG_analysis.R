...
pebbles_het <- subset(PEBBLES_soupx, subset = Genotype == 'HET')
cluster <- pebbles_het
#####################################
## Testing DEGs for each cell type ##
#####################################
# Perform DEG analysis for each age group
cell_types <- unique(cluster@meta.data$predicted.id)
deg_results <- list()

for (cell_type in cell_types) {
  cat("Performing DEG analysis for", cell_type, "\n")
  
  # Subset cells based on cell type
  cell_subset <- subset(cluster, subset = predicted.id == cell_type)
  gene_expression_freq <- WhichCells(cell_subset, subset.name = "all_genes", accept.low = 0.25)
  high_freq_genes <- names(gene_expression_freq[gene_expression_freq >= 0.25])
  cell_subset <- subset(cell_subset, features = high_freq_genes)
  
  # Get expression info
  expr <- as.matrix(GetAssayData(cell_subset))
  ...
  mm <- model.matrix(~0 + Treatment + percent.mito, data = cell_subset@meta.data)
  ...
  deg_results[[cell_type]] <- top_table
}

# Access DEG results for each age group
for (cell_type in cell_types) {
  cat("DEG analysis results for", cell_type, "\n")
  
  deg_table <- deg_results[[cell_type]]
...
deg_results$L2_3_IT$Cell_type <- 'L2_3_IT'
deg_results$Vip$Cell_type <- 'Vip'
deg_results$Oligo$Cell_type <- 'Oligo'
deg_results$L4$Cell_type <- 'L4'
deg_results$Lamp5$Cell_type <- 'Lamp5'
deg_results$L5$Cell_type <- 'L5'
deg_results$Pvalb$Cell_type <- 'Pvalb'
deg_results$Sst$Cell_type <- 'Sst'
deg_results$L6$Cell_type <- 'L6'
deg_results$`Non-neuronal`$Cell_type <- 'Non-neuronal'
deg_results$Astro$Cell_type <- 'Astro'
deg_results$Sncg$Cell_type <- 'Sncg'

DEG_df <- rbind(deg_results$L2_3_IT, deg_results$Vip, deg_results$Oligo, deg_results$L4, deg_results$Lamp5, deg_results$L5, deg_results$Pvalb, deg_results$Sst, deg_results$L6, deg_results$`Non-neuronal`, deg_results$Astro, deg_results$Sncg)
write.csv(DEG_df, file = glue('/Users/osman/Documents/GitHub/PEBBLES_mouse_snRNAseq/soupx/limmaVoomCC/pebbles_DEGs.csv'))
# Assuming 'adj.P.Val' is the column containing adjusted p-values
filtered_df <- DEG_df[DEG_df$adj.P.Val <= 0.05, ]
write.csv(filtered_df, file = glue('/Users/osman/Documents/GitHub/PEBBLES_mouse_snRNAseq/soupx/limmaVoomCC/pebbles_sig_DEGs.csv'))
...
cell_typess <- unique(subset_cell_nonautonomous@meta.data$Age)
deg_results <- list()
...
DEGs = top.table
filtered_df$SYMBOL = rownames(filtered_df)
DEGs = filtered_df
# Top DEGs
DEGs <- fit %>%
