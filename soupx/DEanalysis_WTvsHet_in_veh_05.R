...
cell_types_all = cell_types_all[c(1:12)] # not enough cells for these cell types

for (i in cell_types_all) {
  DGEList_high[[i]]$samples$group = ifelse(grepl("WT", colnames(DGEList_high[[i]]$counts))=="TRUE", "WT", "HET")
}

cell_types_all = cell_types_all[c(2:12)]
...
for (i in cell_types_all) {
  ...
  mm <- model.matrix(~genotype + percent.mito,
