...
mosaic.cortex <- subset(x = PEBBLES_soupx, subset = Group == c('HET-PCB', 'HET-VEHICLE'))
table(mosaic.cortex$Group)
# Perform DEG analysis between the WT cells from the WT mouse and WT cells from the mosaic brains
het_PCBs <- subset(x = mosaic.cortex, subset = Group == 'HET-PCB')
het_VEHICLEs <- subset(x = mosaic.cortex, subset = Group == 'HET-VEHICLE')
WT_from_PCB = Cells(het_PCBs)[which(het_PCBs$Mecp2_allele == "WT_Mecp2")]
MUT_from_VEHICLE = Cells(het_VEHICLEs)[which(het_VEHICLEs$Mecp2_allele == "MUT_Mecp2")]
slct_WT_from_HET = sample(WT_from_PCB, size = 95)
slct_WT_from_WT = sample(MUT_from_VEHICLE, size = 95)
...
directory_path = glue('{base_path}11_WTcellsVsMUTcells_from_MUTPCB_MUTVEHICLE')
...
temp <- venn.diagram(
  ...
  main = 'sig_11_WTcellsVsMUTcells_from_MUTPCB_MUTVEHICLE_DEGs ',
