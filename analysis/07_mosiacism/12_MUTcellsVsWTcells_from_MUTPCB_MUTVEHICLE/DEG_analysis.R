...
mosaic.cortex <- subset(x = PEBBLES_soupx, subset = Group == c('HET-VEHICLE', 'HET-PCB'))
table(mosaic.cortex$Group)

# Perform DEG analysis between the WT cells from the WT mouse and WT cells from the mosaic brains
het_PCB <- subset(x = mosaic.cortex, subset = Group == 'HET-PCB')
het_VEHICLE <- subset(x = mosaic.cortex, subset = Group == 'HET-VEHICLE')

MUT_from_PCB = Cells(het_PCB)[which(het_PCB$Mecp2_allele == "MUT_Mecp2")]
WT_from_VEHICLE = Cells(het_VEHICLE)[which(het_VEHICLE$Mecp2_allele == "WT_Mecp2")]
slct_MUT_from_PCB = sample(MUT_from_PCB, size = 146)
slct_WT_from_VEHICLE = sample(WT_from_VEHICLE, size = 146)
subset_cell_nonautonomous = subset(mosaic.cortex, cells = c(slct_MUT_from_PCB, slct_WT_from_VEHICLE))
table(subset_cell_nonautonomous$Group,subset_cell_nonautonomous$Mecp2_allele)
...
directory_path = glue('{base_path}12_MUTcellsVsWTcells_from_MUTPCB_MUTVEHICLE')
...
temp <- venn.diagram(
  ...
  main = 'sig_MUTcellsVsWTcells_from_MUTPCB_MUTVEHICLE_DEGs ',
