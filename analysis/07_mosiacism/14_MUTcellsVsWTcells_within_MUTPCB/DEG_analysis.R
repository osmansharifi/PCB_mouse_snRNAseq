...
mosaic.cortex <- subset(x = PEBBLES_soupx, subset = Group == 'HET-PCB')
...
slct_WT_from_HET = sample(MUT_from_HET, size = 199)
slct_WT_from_WT = sample(WT_from_HET, size = 199)
...
directory_path = glue('{base_path}14_MUTcellsVsWTcells_within_MUTPCB')
...
temp <- venn.diagram(
  ...
  main = 'sig_MUTcellsVsWTcells_within_MUTPCB',
