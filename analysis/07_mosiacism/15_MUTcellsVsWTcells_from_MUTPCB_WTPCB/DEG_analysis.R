...
mosaic.cortex <- subset(x = PEBBLES_soupx, subset = Group == c('HET-PCB','WT-PCB'))
table(mosaic.cortex$Group)
# Perform DEG analysis between the WT cells from the WT mouse and WT cells from the mosaic brains
cell_nonautonomous_Het <- subset(x = mosaic.cortex, subset = Group == 'HET-PCB')
cell_nonautonomous_WT <- subset(x = mosaic.cortex, subset = Group == 'WT-PCB')
MUT_from_HET = Cells(cell_nonautonomous_Het)[which(cell_nonautonomous_Het$Mecp2_allele == "MUT_Mecp2")]
WT_from_WT = Cells(cell_nonautonomous_WT)[which(cell_nonautonomous_WT$Mecp2_allele == "WT_Mecp2")]
slct_WT_from_HET = sample(MUT_from_HET, size = 182)
slct_WT_from_WT = sample(WT_from_WT, size = 182)
subset_cell_nonautonomous = subset(mosaic.cortex, cells = c(slct_WT_from_HET, slct_WT_from_WT))
...
for (celltype in celltypes) {
  ...
  mm <- model.matrix(~0 + Genotype, data = broad_class_subset@meta.data)
  y <- voom(expr, mm, plot = TRUE)
  fit <- lmFit(y, mm)
  
  # Extract DEG results
  contrasts <- makeContrasts(c(GenotypeHET) - c(GenotypeWT), levels = colnames(coef(fit)))
...
directory_path = glue('{base_path}15_MUTcellsVsWTcells_from_MUTPCB_WTPCB')
...
total_degs <- rbind(deg_results$Glutamatergic, deg_results$GABAergic, deg_results$`Non-neuronal`)
total_degs$SYMBOL <- rownames(total_degs)
sig_total_degs <- subset(total_degs, adj.P.Val <= 0.05)
write.csv(sig_total_degs, file = glue('{directory_path}/sig_DEGs_{dir_name}.csv'))
###########################################
...
temp <- venn.diagram(
  ...
  main = 'sig_MUTcellsVsWTcells_from_MUTPCB_WTPCB_DEGs ',
